...
impl PathNormalizer {
    ...
    pub fn to_normalized_str(path: &Path) -> String {
    ...
    pub fn relative_from_config(&self, path: &Path) -> Option<String> {
        ...
        norm_path
            ...
            .ok()
            .map(|rel: &Path| Self::to_normalized_str(rel))
            .map(|s: String| if s.is_empty() { ".".to_string() } else { s })
    }
...
mod tests {
    ...
    fn test_to_normalized_str() {
        ...
        // Unix paths
        assert_eq!(
            PathNormalizer::to_normalized_str(Path::new("a/b/c")),
            "a/b/c"
        );
